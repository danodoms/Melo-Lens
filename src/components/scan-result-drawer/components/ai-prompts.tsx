...
import { getAiResponseStream2 } from "../../../lib/ai/fetch";
...
export const AiPrompts: React.FC<RenderAiPromptsProps> = ({
  drawerState,
  setAiSession,
}) => {
  const prompts = [
  ...
  const handleAiPrompt = async (prompt: string) => {
    ...
    let responseStream = "";
    await getAiResponseStream2(generatePrompt(prompt), (chunk: string) =>
      setAiSession({
        prompt,
  ...
  return (
    <VStack className="w-full mt-8">
      ...
      <HStack className="max-h-64 flex-wrap gap-2 pb-16 overflow-auto">
