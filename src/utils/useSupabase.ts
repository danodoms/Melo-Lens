...
export const useSupabase = () => {
   ...
   */
  const uploadImagesToBucket = async (
    bucketName: string,
    directory: string
  ) => {
    try {
      ...
      for (const file of jpgFiles) {
        ...
        if (existingFiles.some((f) => f.name === fileName)) {
          console.log(
            `File "${fileName}" already exists for user "${userId}". Skipping upload.`
          );
          continue;
        }
  ...
  const syncLocalImagesToRemoteDatabase = async () => {
    try {
      ...
      return { success: true, message: "Images synchronized successfully" };
    } catch (error) {
      console.error("Error in syncLocalImagesToRemoteDatabase:", error);
      return { success: false, message: "Failed to sync images", error };
    }
  };
